...
def determine_move3(thresh):
    ...
    for i in range(64):
        sq = squares[i]
        tempSum = 0
        for y in range(sq[0][1],sq[2][1]):
            for x in range(sq[0][0],sq[1][0]):
                    s[i]+=thresh[y][x]
    ...
    first=str(chr(ord('h')-ind1%8)+str(1+(ind1//8)))
    second=str(chr(ord('h')-ind2%8)+str(1+(ind2//8)))
    print('Squares are')
    print(squares[i])
    for i in range(64):
        s = squares[i]
        cv2.rectangle(thresh, s[0],s[2], (255,0,0), 2)

    cv2.imshow('threshSquares',thresh)
    #return str(first)+str(second)
    return (first,second)

def determine_move4(init,final):
    frameDelta = cv2.absdiff(init, final)
    ...
    cv2.imshow('threshSquares',thresh)
    blur=cv2.GaussianBlur(init,(5,5),0)
    gray=cv2.cvtColor(init, cv2.COLOR_BGR2GRAY)
    final=np.zeros(init.shape)
    _,binar=cv2.threshold(gray,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    binar1 = cv2.Canny(init,100,120)
    kernel = np.ones((3,3),np.uint8)
    dil1 = cv2.dilate(binar,kernel,iterations = 1)
    blur=cv2.GaussianBlur(final,(5,5),0)
    gray=cv2.cvtColor(final, cv2.COLOR_BGR2GRAY)
    final=np.zeros(final.shape)
    _,binar=cv2.threshold(gray,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    binar2 = cv2.Canny(final,100,120)
    _,conts,hier=cv2.findContours(binar,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(final, conts, -1, (0,255,0), 3)
    kernel = np.ones((3,3),np.uint8)
    dil2 = cv2.dilate(binar,kernel,iterations = 1)
...
def determine_move2(cur,final):
    img1 = chess_corners_HSV(cur)[0]
    img2 = chess_corners_HSV(final)[0]
    lu = (0,0)
    ru = (320,0)
    ld = (0,320)
    rd = (320,320)
